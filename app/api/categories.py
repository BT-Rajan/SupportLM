...
def create_category(req: CategoryIn, tenant_id: int = Depends(require_role("editor"))):
...
def delete_category(category_id: int, tenant_id: int = Depends(require_role("admin"))):
    with get_conn() as conn:
        cur = conn.cursor()
